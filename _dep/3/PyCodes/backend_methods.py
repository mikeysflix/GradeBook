...
class GradeBookVisualizer(StudentInteractionMethods):
    ...
    def view_bipartite_network_graph(self, layout_id, scheme, cmap='jet', save=False, **kwargs):
        if layout_id not in ('classic', 'circular', 'arc'):
            raise ValueError("invalid layout_id: {}".format(layout_id))
        cls = BiPartiteNetworkVisualSchematics(
            graph=self.graph,
        ...
        if layout_id == 'classic':
            ...
            raise ValueError("invalid layout_id: {}".format(layout_id))
        ## show/save figure
        savename = 'bipartite_grade_distribution_{}_{}'.format(layout_id, scheme.replace(' ', '_').replace('-', '_')) if save else None
        self.visual_configuration.display_image(fig, savename)
