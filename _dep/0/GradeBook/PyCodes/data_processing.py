from gradebook_methods import *
from visual_configuration import *

class GradeBook(GradeBookEmailMethods):

    def __init__(self, fpath, index_by, name_loc=None, id_loc=None, email_loc=None, homework_loc=None, exam_loc=None, extra_credit_loc=None, homework_weight=None, exam_weight=None, extra_credit_weight=None, bias='left', savedir=None):
        """
    ...
    def __call__(self, fail_score=None, ace_score=None, decimals=None, flat_curve=None, homework_curve=None, exam_curve=None, extra_credit_curve=None, max_curve=np.inf):
        """
    ...
    def view_distribution_heatmap(self, sources=None, headers=None, distance_metric='manhattan', show_dissimilarity=False, show_ticklabels=False, cmap='Oranges', textcolors=('white', 'k'), mask_diagonal=False, interpolation='nearest', save=False, **kwargs):
        ...
        if headers is None:
            ...
            for source in sources:
                if source == 'grade':
                    distance_matrix = self.distance_matrix.get_distance_matrix(self.final['score'], mask_diagonal)
                ...
                for source in ('homework', 'exam', 'extra credit', 'curve'):
                    if source in list(self.points.keys()):
                        if header == 'total':
                            ...
                            distance_matrix = self.distance_matrix.get_distance_matrix(scores, mask_diagonal)
                            norm = Normalize(vmin=0, vmax=np.nanmax(distance_matrix))
