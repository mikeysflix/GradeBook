...
from subroutine_methods import *

class GradeBookViewer(SubRoutines):

    def __init__(self, savedir=None, ticksize=7, labelsize=8, textsize=5, titlesize=9, headersize=10, cellsize=15, bias='left'):
        """
    ...
    def subview_histogram(self, xticks, counts, cmap, default_color, title, **kwargs):
        """

        """
        ## get bin parameters
        midpoints = (xticks[1:] + xticks[:-1])/2
        width = np.diff(xticks)
    ...
    def view_grades_distribution_histogram(self, show_statistics=False, percents=None, cmap='Oranges', default_color='darkorange', bin_width=None, save=False, **kwargs):
        """
        ...
        fig, ax, midpoints, facecolors = self.subview_histogram(xticks, counts, cmap, default_color, title, **kwargs)
        ...
        if xticklabels is not None: # cannot use custom bin-size
            for x,y,s,c in zip(midpoints, counts, self.letters, facecolors):
                ax.text(x, y +0.325, s, fontsize=self.labelsize, color=c, ha='center')
            ax.set_xticklabels(xticklabels, rotation=15)
        if percents is not None:
            percents = self.select_percents(percents)
            if bin_width is None:
                ploc = percents * self.grading_criteria['weight'] / 100
                ...
                for i, (p, loc) in enumerate(zip(percents, ploc)):
                    test_score = p * self.grading_criteria['weight'] / 100
                ...
                ploc = percents * self.grading_criteria['weight'] / 100
            self.subview_percents(ax, percents, ploc, mirror_axis='x', limit=None)
        ## show statistics via legend
        if show_statistics:
            self.subview_statistics(fig, ax, self.final['statistics'], loc=None, decimals=2)
        ## show/save figure
        savename = 'histogram_distribution_grade' if save else None
        self.display_image(fig, savename)

    def view_assignment_distribution_histogram(self, source, header, loc, score=None, show_statistics=False, percents=None, cmap='Oranges', default_color='darkorange', bin_width=None, save=False, **kwargs):
        """
        ...
        ## automatic bin-size
        if bin_width is None:
            edges = self.get_histogram_edges_from_weight(weight)
        ## customized bin-size
        else:
        ...
        fig, ax, _, _ = self.subview_histogram(xticks, counts, cmap, default_color, title, **kwargs)
        ## show percents
        if percents is not None:
            percents = self.select_percents(percents)
        ...
        if show_statistics:
            ...
            self.subview_statistics(fig, ax, statistics, i, decimals=2)
        ## show/save figure
        if save:
    ...
    def view_distribution_histogram(self, sources=None, headers=None, show_statistics=False, percents=None, cmap='Oranges', default_color='darkorange', bin_width=None, save=False, **kwargs):
        """
    ...
    def subview_heatmap(self, source, header, distance_matrix, distance_metric, norm, show_dissimilarity=False, show_ticklabels=False, cmap='Oranges', textcolors=('white', 'k'), interpolation='nearest', title=None, save=False, **kwargs):
        ...
        if show_ticklabels:
            if 'id number' in list(self.identifiers.keys()):
                ...
                ax.set_xlabel('ID number', fontsize=self.labelsize)
                ax.set_ylabel('ID number', fontsize=self.labelsize)
            else:
        ...
        if show_dissimilarity:
            ...
            kws = dict(ha='center', va='center', weight='semibold', fontsize=self.textsize)
        ...
        ax.tick_params(axis='both', labelsize=self.ticksize)
        ## show color-bar
        cbar = fig.colorbar(handle, ax=ax, cmap=cmap, norm=norm, orientation='vertical', shrink=0.5, pad=0.1, extend='max')
        cbar.ax.tick_params(labelsize=self.ticksize)
        cbar.ax.set_title('Dis-Similarity\nof Scores\n({} Distance)'.format(distance_metric.title()), fontsize=self.labelsize)
        ## show title
    ...
    def view_distribution_heatmap(self, sources=None, headers=None, distance_metric='manhattan', show_dissimilarity=False, show_ticklabels=False, cmap='Oranges', textcolors=('white', 'k'), mask_diagonal=False, interpolation='nearest', save=False, **kwargs):
        ...
        if headers is None:
            ...
            for source in sources:
                if source == 'grade':
                    distance_matrix = self.get_distance_matrix(self.final['score'], distance_metric, mask_diagonal)
                ...
                for source in ('homework', 'exam', 'extra credit', 'curve'):
                    if source in list(self.points.keys()):
                        if header == 'total':
                            ...
                            distance_matrix = self.get_distance_matrix(scores, distance_metric, mask_diagonal)
                            norm = Normalize(vmin=0, vmax=np.nanmax(distance_matrix))
    ...
    def view_distribution_stacks(self, sources=None, headers=None, percents=None, identifier_label=None, cmap='jet', edgecolor=None, width=0.8, differentiate_sources=False, save=False, **kwargs):
        ...
        if headers is None:
            ...
            facecolors = iter(self.select_facecolors(np.arange(nstacks), cmap, default_color=None))
            ## iteratively plot
            for source in sources:
                if differentiate_sources:
        ...
        if identifier_label is None:
            ...
            ax.set_xlabel('Students', fontsize=self.labelsize)
        else:
            ...
            ax.set_xticklabels(xticklabels, fontsize=self.ticksize, rotation=15)
            if identifier_label == 'id number':
                ax.set_xlabel('Student ID Number'.format(identifier_label), fontsize=self.labelsize)
            else:
                ax.set_xlabel('Student {}'.format(identifier_label.title()), fontsize=self.labelsize)
        ## configure y-axis
        ax.yaxis.set_minor_locator(ticker.AutoMinorLocator())
        ylim = ax.get_ylim()
        ylim_prime = [0, ylim[-1] * 1.25]
        ax.set_ylim(ylim_prime)
        ax.set_ylabel('Points', fontsize=self.labelsize)
        ## show percents
        if percents is not None:
            percents = self.select_percents(percents)
            ploc = percents * self.grading_criteria['weight'] / 100
            self.subview_percents(ax, percents, ploc, mirror_axis='y', limit=ylim_prime)
        ## finalize plot
        ax.tick_params(axis='both', labelsize=self.ticksize)
        ax.grid(color='k', linestyle=':', alpha=0.3)
        # ax.set_title(title, fontsize=self.titlesize)
        handles, labels = ax.get_legend_handles_labels()
        self.subview_legend(fig, handles, labels, self.get_empty_handle(ax), title='Distribution of Points')
        ## show/save figure
        self.display_image(fig, savename)

    def view_distribution_boxplots(self, sources, percentiles=(25, 75), cmap='Oranges', default_color='darkorange', showmeans=False, showfliers=False, group_by_points=False, save=False, **kwargs):
        """
        ...
        for source in sources:
            ...
            if not group_by_points:
                ...
                ax.set_yticklabels(yticklabels, fontsize=self.ticksize)
            ax.yaxis.set_minor_locator(ticker.AutoMinorLocator())
            ax.tick_params(axis='both', labelsize=self.ticksize)
            ax.grid(color='k', linestyle=':', alpha=0.3)
            ## initialize axis labels
            ax.set_xlabel('Source of Points', fontsize=self.labelsize)
            if group_by_points:
                ax.set_ylabel('Student Score', fontsize=self.labelsize)
            else:
                ax.set_ylabel('Percentage of\nStudent Score', fontsize=self.labelsize)
