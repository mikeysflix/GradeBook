...
class DataInitialization():
    ...
    def __init__(self):
        ...
        self.allowed_fpath_extensions = ('.csv', '.txt', '.xlsx', '.xls')
    ...
    def initialize_raw_data(self, fpath):
        ...
        if extension in self.allowed_fpath_extensions:
            if extension in ('.csv', '.txt'):
                ...
                raise ValueError("extension {} is not yet accounted for")
        else:
        ...
        for key, i in zip(('name', 'id number', 'email'), (name_loc, id_loc, email_loc)):
            if i is not None:
                if isinstance(i, int):
                    ...
                    raise ValueError("invalid type(i): {}".format(type(i)))
...
class GradePointAnalysis(DataInitialization):
    ...
    def initialize_students(self):
        ...
        self._students = np.array(self._students) #, dtype=object)

    def initialize_grade_counter(self):
        self._grade_counter = Counter(self.final['grade'])

    def initialize_network_graph(self):
        """

        """
        student_nodes = np.arange(self.students.size, dtype=int)
        graph = nx.OrderedGraph() # nx.Graph()
        graph.add_nodes_from(self.letters, graph_id=0)
        graph.add_nodes_from(student_nodes, graph_id=1)
        for i, student in enumerate(self.students):
            edge = (i, student.points['final']['grade'])
            graph.add_edge(*edge)
        self._graph = graph
